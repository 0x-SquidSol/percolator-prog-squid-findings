...
mod layout_constants {
    ...
    pub const ACCOUNT_FEES_EARNED_TOTAL_OFF: usize =
        core::mem::offset_of!(percolator::Account, fees_earned_total);
    ...
    pub const ENGINE_NEXT_ACCOUNT_ID_OFF: usize =
        core::mem::offset_of!(percolator::RiskEngine, next_account_id);
...
fn layout_account_field_offsets_pinned() {
    ...
    assert_eq!(
        offset_of!(Account, fees_earned_total),
        layout_constants::ACCOUNT_FEES_EARNED_TOTAL_OFF,
        "Account.fees_earned_total offset changed: {} vs pinned {}",
        offset_of!(Account, fees_earned_total),
        layout_constants::ACCOUNT_FEES_EARNED_TOTAL_OFF,
    );
}
...
fn layout_engine_field_offsets_pinned() {
    ...
    assert_eq!(
        ...
        layout_constants::ENGINE_NEXT_ACCOUNT_ID_OFF,
        "RiskEngine.next_account_id offset drift"
    );
    assert_eq!(
...
#[test]
fn account_account_id_at_offset_zero() {
    assert_eq!(
        offset_of!(Account, account_id),
        0,
        "account_id must be the first field at offset 0"
    );
}

#[test]
fn account_capital_immediately_follows_account_id() {
    assert_eq!(
        offset_of!(Account, capital),
        offset_of!(Account, account_id) + size_of::<u64>(),
        "capital must immediately follow account_id (offset 8)"
    );
}

#[test]
fn account_kind_immediately_follows_capital() {
    // capital is U128 = [u64; 2] = 16 bytes, starting at offset 8. So kind = 24.
    assert_eq!(
        offset_of!(Account, kind),
        24,
        "Account.kind must be at offset 24"
    );
}
...
fn account_fields_do_not_overlap() {
    ...
    assert!(
        size_of::<Account>() >= offset_of!(Account, fees_earned_total) + 16,
        "Account struct is too small to contain fees_earned_total"
    );
}
